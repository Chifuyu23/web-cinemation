      ...
      <div class="w-full h-auto min-h-screen flex flex-col font-inter z-10">
        ...
        <div class="w-full flex flex-row mt-16 mb-20 px-10">
          ...
          <div class="flex flex-col w-full ml-12 bg-white p-10 rounded-2xl drop-shadow-[0_0px_2px_rgba(0,0,0,0.2)] overflow-x-scroll">
            <form id="dataForm" method="POST" action="{{ route('movies.update') }}" enctype="multipart/form-data" class="flex flex-col">
              @csrf
              @method('PUT')
              <div class="font-semibold text-3xl">Edit Data</div>
              ...
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2">
                <input type="text" name="title" maxlength="255" class="outline-none w-full font-normal" value="{{ old('title', $data->title) }}" autocomplete="off" required/>
              </div>

              <div class="font-semibold text-2xl mt-7">Deskripsi <span class="text-red-500">*</span></div>
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2">
                <textarea name="overview" rows="5" class="outline-none w-full font-normal resize-none" required>{{ old('overview', $data->overview) }}</textarea>
              </div>

              <div class="font-semibold text-2xl mt-7">Tanggal Rilis <span class="text-red-500">*</span></div>
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2">
                <input id="datePicker" type="date" name="release_date" class="outline-none w-full font-normal bg-white" value="{{ old('release_date', $data->release_date) }}" onclick="this.showPicker()" required/>
              </div>

              <div class="font-semibold text-2xl mt-7">Rating (1-10) <span class="text-red-500">*</span></div>
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2">
                <input type="number" name="vote_average" class="outline-none w-full font-normal" value="{{ old('vote_average', $data->vote_average) }}" autocomplete="off"
                  max="10"
                  min="1"
                  required/>
              </div>
              ...
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2">
                <input type="number" name="runtime" class="outline-none w-full font-normal" value="{{ old('runtime', $data->runtime) }}" autocomplete="off"
                  min="1"
                  required/>
              </div>
              ...
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2">
                <input type="text" name="youtube_id" maxlength="255" class="outline-none w-full font-normal" value="{{ old('youtube_id', $data->youtube_id) }}" autocomplete="off" required/>
              </div>

              <div class="font-semibold text-2xl mt-7">Tipe <span class="text-red-500">*</span></div>
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2">
                <select class="pr-2 py-2 w-full" name="type">
                  <option value="movie" @selected(old('type', $data->type) == 'movie')>Movie</option>
                  <option value="tv_show" @selected(old('type', $data->type) == 'tv_show')>TV Show</option>
                </select>
              </div>

              <div class="font-semibold text-2xl mt-7">File Backdrop (Max {{ env('MAX_FILE_SIZE') }} MB)</div>
              <div class="flex items-center justify-left w-full mt-2">
                <input id="backdrop_file" type="file" name="backdrop_file" accept="image/png, image/jpeg, image/jpg" value="{{ old('backdrop_file') }}" onchange="validateSize(this, {{ env('MAX_FILE_SIZE') }}, '#backdrop_file')"/>
              </div>
              <x-input-error :messages="$errors->get('backdrop_file')" class="mt-2" />

              <div class="font-semibold text-2xl mt-7">File Poster (Max {{ env('MAX_FILE_SIZE') }} MB)</div>
              <div class="flex items-center justify-left w-full mt-2">
                <input id="poster_file" type="file" name="poster_file" accept="image/png, image/jpeg, image/jpg" value="{{ old('poster_file') }}" onchange="validateSize(this, {{ env('MAX_FILE_SIZE') }}, '#poster_file')"/>
              </div>

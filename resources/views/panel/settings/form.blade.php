      ...
      <div class="w-full h-auto min-h-screen flex flex-col font-inter z-10">
        ...
        <div class="w-full flex flex-row mt-16 mb-20 px-10">
          ...
          <div class="flex flex-col w-full ml-12 bg-white p-10 rounded-2xl drop-shadow-[0_0px_2px_rgba(0,0,0,0.2)] overflow-x-scroll">
            <form id="dataForm" method="POST" action="{{ route('settings.update') }}" enctype="multipart/form-data" class="flex flex-col">
              @csrf
              @method('PUT')
              <div class="font-semibold text-3xl">Pengaturan Data Profil</div>
              ...
              <div class="font-semibold text-2xl mt-7">Nama <span class="text-red-500">*</span></div>
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2">
                <input type="text" name="name" maxlength="50" class="outline-none w-full font-normal" value="{{ old('name', $user->name) }}" required/>
              </div>

              <div class="font-semibold text-2xl mt-7">Email</div>
              <div class="w-full border border-[rgba(0,0,0,0.5)] rounded-lg font-semibold text-xl p-4 mt-2 bg-gray-100 cursor-not-allowed">
                <input type="text" name="email" maxlength="20" class="outline-none w-full font-normal bg-transparent cursor-not-allowed" value="{{ old('email', $user->email) }}" readonly/>
              </div>
